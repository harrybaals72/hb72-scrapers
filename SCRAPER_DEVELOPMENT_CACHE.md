...
`fc2madb.py` reads `fc2cmadb_email` and `fc2cmadb_password` from its ignored `config.ini`, never reads a browser cookie export, and destroys the temporary FlareSolverr session after the credential POST. It first tries its own ignored `auth_session.json`; only a missing/invalid/unauthenticated saved session triggers credential login. Login-only Inertia/XSRF headers must be scoped to that POST rather than stored in `session.headers`. After login or article responses, collapse FlareSolverr/domain cookies to one `fc2cmadb.com`/`/` identity per name, preferring fresh response values; otherwise Requests can send both a stale domain cookie and a fresh host-only cookie and Laravel may select the stale session. Persist cookies atomically with restrictive permissions only after authenticated article props are proven. It records each origin phase in rate state, labels missing-header diagnostics by phase/source, honors cooldowns before subsequent requests, and forces/persists a 30-second cooldown on every 429.
